...
def fullMonty10digit(loadNet):
    ...
    if loadNet!=1:
        print("training...")
        #read data from train file
        d.createbatch('digits_train.txt',10) 
        print("End reading data and Start training")

        #train NN param:input, target, eta, mu, niter, report
        d.train(d.batchI,d.batchT,0.75,0,iter,100) 
        print("End training and start reading testing data")
        d.save("NN10digit")
    else:
        d.load("NN10digit")
    #read data from test file
    d.createbatch('digits_test.txt',10)
    print("End reading testing data and start testing")
    d.showConfusion()
    return d
...
if __name__ == '__main__':
    ...
    #in part 3 use fullMonty10digit(1) to load "NN10digit" NN
    d=fullMonty10digit(1)
    print("Current Time:",datetime.datetime.now().time())
